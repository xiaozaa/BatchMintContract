...
contract Deployed is ReentrancyGuard {
    ...
    function priceSale() public pure returns (uint) {}
...
contract contractMint is ReentrancyGuard , IERC721Receiver, Ownable{
    ...
    function setParser(address _t) external onlyOwner {
        parser = _t;
    }
    ...
    function mint(address _e, uint256 _val) external payable onlyOwner{
        (bool success, ) = _e.call{value: _val * getPrice()}(abi.encodePacked(bytes4(keccak256("saleMint(uint256)")), _val)); // D's storage is set, E is not modified 
        require(success);
    }
    ...
    function withdrawNFT(address _e, uint256 id) external nonReentrant onlyOwner {
